...
export class StampsStorage {
    ...
    public static update(uuid : TStamp['uuid'], stamp : TStamp)
    {
        localStorage.setItem(this.STAMPS_LOCALSTORAGE_KEY, JSON.stringify([...this.getCurrent().filter(stp => stp.uuid !== uuid), {...stamp, uuid}]))
    }
